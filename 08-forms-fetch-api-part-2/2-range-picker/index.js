export default class RangePicker {
  ...
  updateCalendarButtons(countOfDays, currentDate) {
    const buttons = countOfDays.map((day, index) => {
      return `<button type="button" class="rangepicker__cell" data-value="${getDataValue(currentDate, index + 1)}" style="--start-from: 5">${day}</button>`
    }).join('');

    function getDataValue(date, day) {
  ...
  updateDateInput(from, to) {
    return `
            <span data-element="from">${from.toLocaleDateString()}</span> -
            <span data-element="to">${to.toLocaleDateString()}</span>
          `
  }
    ...
    return `
                <div class="container">
                        <div data-element="rangepicker" class="rangepicker">
                                <div data-element="picker-input" class="rangepicker__input" data-element="input">
                                  ${this.updateDateInput(this.from, this.to)}
                                </div>
  ...
  pickDate = (event) => {
    ...
    if (cell) {
      if (this.selectedDate) {
        ...
        // update input date
        from.textContent = (new Date(this.from)).toLocaleDateString(); //TODO Вынести в отдельный метод
        to.textContent = (new Date(this.to)).toLocaleDateString();
